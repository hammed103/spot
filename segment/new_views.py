...
class start(APIView):
    ...
    def get(req):
        ...
        df[['total_active_audience_listeners', 'super_active_audience_listeners', 'moderate_active_audience_listeners', 'light_active_audience_listeners', 'programmed_audience_listeners', 'previously_active_audience_listeners']]  = df["segments"].apply(extract_audience_info)

        df[['total_active_audience_streams', 'super_active_audience_streams', 'moderate_active_audience_streams', 'light_active_audience_streams', 'programmed_audience_streams', 'previously_active_audience_streams']]  = df["streams"].apply(extract_audience_info)

        df["%active_audience_listeners"]   = ((df.total_active_audience_listeners / (df.total_active_audience_listeners + df.programmed_audience_listeners + df.previously_active_audience_listeners)) * 100).round()
        df["%previously_active_audience_listeners"]   = ((df.previously_active_audience_listeners / (df.total_active_audience_listeners + df.programmed_audience_listeners + df.previously_active_audience_listeners)) * 100).round()
        df["%programmed_audience_listeners"]   = ((df.programmed_audience_listeners / (df.total_active_audience_listeners + df.programmed_audience_listeners + df.previously_active_audience_listeners)) * 100).round()

        df["%active_audience_streams"]   = ((df.total_active_audience_streams / (df.total_active_audience_streams + df.programmed_audience_streams + df.previously_active_audience_streams)) * 100).round()
        df["%previously_active_audience_streams"]   = ((df.previously_active_audience_streams / (df.total_active_audience_streams + df.programmed_audience_streams + df.previously_active_audience_streams)) * 100).round()
        df["%programmed_audience_streams"]   = ((df.programmed_audience_streams / (df.total_active_audience_streams + df.programmed_audience_streams + df.previously_active_audience_streams)) * 100).round()
        ...
        oki = df[df.Date.isin([str(date.today() - timedelta(4)),str(date.today() - timedelta(3)),str(date.today() - timedelta(2)),str(date.today() - timedelta(1))])]
        ...
        for id,namex in art[:2] :
          for cd,country_name in countries[:2] :
            ...
            reshaped_df["age"] =  reshaped_df["age"].apply(lambda x: x.rstrip("_gender").lstrip("age_").replace("_","-"))
            bov = reshaped_df[["Date","gender","age","country","listeners","artist_id","artist_name"]]
            lb.append(bov)
